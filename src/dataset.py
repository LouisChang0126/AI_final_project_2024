...
def read_image(gray_img = False, coordinate = False):
    ...
    folder_paths = ['../data/~1500', '../data/1500~1800', '../data/1800~1970', '../data/1970~']
    for folder_path in folder_paths:
        ...
        for filename in df['picture']:
            ...
            if gray_img:
                image_array = np.array(cv2.resize(cv2.imread(filepath, cv2.IMREAD_GRAYSCALE),(256,256)))
            else:
                image_array = np.array(cv2.resize(cv2.imread(filepath),(256,256)))
            if coordinate:
